...
async function salvaArbitro(req, res) {
  const body = req.body;
  ...
  const utente = new Utente({
    ...
    codicePatentino: body.codicePatentino,
    approvato: false,
    ruolo: 'arbitro'
  })
  ...
  utente.save()
  .then(() => {
    res.status(201).location('/api/v1/utenti/' + utente._id).send({success: true, self: '/api/v1/utenti/' + utente._id})
  })
  .catch((error) => {
    console.log(error)
